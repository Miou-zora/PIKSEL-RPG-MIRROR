...
void set_pourcent_color(float *pourcent, sfVector2i *pixel_pos,
sfColor *color, framebuffer_t *framebuffer)
{
    if (*pourcent > 1)
        *pourcent = 1;
    color->r *= (1 - *pourcent);
    ...
    color->b *= (1 - *pourcent);
    color->a *= (1 - *pourcent);
    framebuffer->pixels[(pixel_pos->x + pixel_pos->y * framebuffer->width) *
    4 + 0] = color->r;
    framebuffer->pixels[(pixel_pos->x + pixel_pos->y * framebuffer->width) *
    4 + 1] = color->g;
    framebuffer->pixels[(pixel_pos->x + pixel_pos->y * framebuffer->width) *
...
framebuffer_t *framebuffer, sfVector2f offset, sfColor color)
{
    ...
    distance = sqrt(pow(particle->position.x, 2) +
    pow(particle->position.y, 2));
    pourcent = pow((distance * 100.f / (framebuffer->height / 2)) /
    100.f, 2);
    set_pourcent_color(&pourcent, &pixel_pos, &color, framebuffer);
}
