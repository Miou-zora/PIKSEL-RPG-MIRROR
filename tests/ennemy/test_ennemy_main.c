...
void init_sprite(game_t *game)
{
    ...
    game->ennemy->max_left = 580;
    game->ennemy->clock = sfClock_create();
    game->ennemy->sprite_data = malloc(sizeof(sprite_data_t));
    game->ennemy->sprite_data->texture =
    sfTexture_createFromFile("assets/ennemy/wolf.png", NULL);
    game->ennemy->sprite_data->sprite = sfSprite_create();
    game->ennemy->sprite_data->pos = (sfVector2f){250, 250};
    game->ennemy->sprite_data->scale = (sfVector2f){1.5, 1.5};
...
void manage_mouse_click(sfMouseButtonEvent mouseButton, game_t *game)
{
    ...
    game->distance[0] = (int)(game->ennemy->sprite_data->pos.x - game->click[0]);
    game->distance[1] = (int)(game->ennemy->sprite_data->pos.y - game->click[1]);
}
...
ennemy_t *move_texture(ennemy_t *ennemy, int left, int height, sfClock *clock)
{
    ...
    if (time > 100000) {
        ...
        if (ennemy->max_left >= ennemy->sprite_data->rect.left + left) {
            ennemy->sprite_data->rect.left += left;
        } else {
            ennemy->sprite_data->rect.left = 0;
        }
        sfSprite_setTextureRect(ennemy->sprite_data->sprite,
        ennemy->sprite_data->rect);
...
ennemy_t *move_ennemy(sfClock *clock, ennemy_t *ennemy, int click[2], int distance[2])
{
    ...
    if ((float)((float)time / (float)100 >= 1)) {
        if (ennemy->sprite_data->pos.x < click[0]
        || ennemy->sprite_data->pos.x > click[0]) {
            ennemy->sprite_data->pos.x -= (float)distance[0] / (float)(abs(distance[1]) + (float)abs(distance[0]));
            if (distance[0] > 0)
                ennemy = move_texture(ennemy, 148, 0, ennemy->clock);
            else
                ennemy = move_texture(ennemy, 148, 115, ennemy->clock);
        }
        if (ennemy->sprite_data->pos.y < click[1]
        || ennemy->sprite_data->pos.y > click[1]) {
            ennemy->sprite_data->pos.y -= (float)distance[1] / (float)(abs(distance[1]) + (float)abs(distance[0]));
        }
        sfSprite_setPosition(ennemy->sprite_data->sprite, ennemy->sprite_data->pos);
        sfClock_restart(clock);
    }
    return (ennemy);
}
